...
class Arc {
  ...
  public:
    ...
    // Added for graph data export - access arc delay values
    std::optional<float> delay(Split el, Tran rf_from, Tran rf_to) const;
...
// Function: delay
// Returns the arc delay for given split (min/max) and transitions (from_tran -> to_tran)
inline std::optional<float> Arc::delay(Split el, Tran rf_from, Tran rf_to) const {
  return _delay[el][rf_from][rf_to];
}
