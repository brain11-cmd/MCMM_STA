...
`losses/losses.py` 中定义了 5 项复合损失（另有负延迟与到达时间约束项）：
...
| **L_worst** | Top-k endpoint `Huber(slack_hat-slack_true)` 均值（优先 `slack_true<0` 端点） | 关键端点优化：优先修复违例端点，再兼顾尾部误差 | λ=0.2 |

**总损失**: `L = L_slack + 0.3·L_edge + 0.01·L_KL + 0.001·L_ent + 0.2·L_worst (+ λ_neg·L_neg + λ_at·L_at)`
